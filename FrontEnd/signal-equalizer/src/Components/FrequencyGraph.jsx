...
const generateAudiogramData = (frequencies, magnitudes) => {
    ...
    // Dynamically detect the actual range from the data
    ...
    audiometricFrequencies.forEach(freq => {
        ...
        frequencies.forEach((fftFreq, index) => {
...
const FrequencyGraph = ({
    ...
    const audiogramData = useMemo(() => {
        if (sanitizedFrequencies.length > 0 && sanitizedMagnitudes.length > 0) {
            console.log("Generating audiogram from frequencies:", sanitizedFrequencies.slice(0, 5),
                "magnitudes:", sanitizedMagnitudes.slice(0, 5));
    ...
    useEffect(() => {
        if (hasFrequencyData && (frequencies.length > 0 || magnitudes.length > 0)) {
            ...
            console.log("Current magnitude range:", Math.min(...sanitizedMagnitudes), "to", Math.max(...sanitizedMagnitudes));
        }
    ...
    useEffect(() => {
        ...
        if (activeScale === 'audiogram' && hasAudiogramData) {
            console.log("Drawing audiogram with data:", audiogramData);
