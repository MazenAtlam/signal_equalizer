    ...
    const animate = (currentTimeStamp) => {
      ...
      setCurrentTime(prevTime => {
        ...
        if (newTime >= duration) {
          ...
          setStatus("Paused");
          if (onPlaybackUpdate) {
            onPlaybackUpdate(newTime, false);
          }
          return newTime;
        }

        // Notify parent of playback update
        if (onPlaybackUpdate) {
    ...
    if (timeSeries.length > 0) {
      ...
      if (visibleSamples.length > 0) {
        ...
        if (currentTime >= visibleStartTime && currentTime <= visibleStartTime + visibleDuration) {
          const position = (currentTime - visibleStartTime) / visibleDuration;
  ...
  return (
      <Card className="cine-viewer col-10 mx-auto">
        ...
        <div className="cine-grid px-4 d-flex gap-3 my-4">
          ...
          <Card className="cine-panel col-6">
            <div className="panel-header d-flex justify-content-between align-items-center ps-4 pt-4 pb-2 col-9">
              <h4 className="panel-title m-0 h-6 col-7" style={{ fontSize: "13px" }}>
                Output Signal
              </h4>
              <div className="status-indicator d-flex align-items-center gap-2">
                <div
